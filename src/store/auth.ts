...
const readPersisted = (): Pick<AuthState, 'user' | 'token'> => {
  try {
    ...
    if (!text) return { user: null, token: null };
...
export const useAuthStore = create<AuthState>((set) => {
  const persisted = typeof window !== 'undefined' ? readPersisted() : { user: null, token: null };
  return {
    user: persisted.user,
    ...
    setToken: (token) => {
      ...
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ user: current.user, token }));
      set({ token, isAuthenticated: Boolean(token && current.user) });
    }
  };
});
